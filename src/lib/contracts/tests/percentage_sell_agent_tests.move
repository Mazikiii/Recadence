...
module recadence::percentage_sell_agent_tests {
    ...
    fun test_create_percentage_sell_agent_success(admin: signer, user: signer) {
        setup_test_env();
        init_aptos_coin();
        ...
        let agent_name = b"Test Percentage Sell Agent";

        // Create Percentage Sell agent - should succeed without error
        percentage_sell_agent::create_percentage_sell_agent_for_testing(
            &user,
            source_token,
            sell_amount_tokens,
    ...
    #[test(admin = @0x1111, user = @0x2222)]
    fun test_percentage_sell_agent_lifecycle(admin: signer, user: signer) {
        setup_test_env();
        init_aptos_coin();
        let dummy_user = account::create_signer_for_test(@0x3333);
        setup_accounts(&admin, &user, &dummy_user);
    ...
    #[test(admin = @0x1111, user = @0x2222)]
    fun test_multiple_percentage_sell_agents(admin: signer, user: signer) {
        setup_test_env();
        init_aptos_coin();
        let dummy_user = account::create_signer_for_test(@0x3333);
        setup_accounts(&admin, &user, &dummy_user);
        ...
        while (i < 3) {
            ...
            vector::append(&mut name, b"Percentage Sell Agent ");
    ...
    #[test(admin = @0x1111, user = @0x2222)]
    fun test_percentage_sell_agent_with_stop_date(admin: signer, user: signer) {
        setup_test_env();
        init_aptos_coin();
        let dummy_user = account::create_signer_for_test(@0x3333);
        setup_accounts(&admin, &user, &dummy_user);
        ...
        let source_token = create_mock_token_metadata();
        let sell_amount_tokens = 200000000; // 2 APT
        let percentage_threshold = 1500; // 15%
        let initial_token_deposit = 2000000000; // 20 APT

        let current_time = timestamp::now_seconds();
        let stop_date = option::some(current_time + (30 * 24 * 3600)); // 30 days from now

        // Create Percentage Sell agent with stop date
        percentage_sell_agent::create_percentage_sell_agent_for_testing(
            &user,
            source_token,
            sell_amount_tokens,
